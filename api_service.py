...
class ChatResponse(BaseModel):
    ...
    suggested_questions: List[str] = Field(default_factory=list, description="3-4 contextual follow-up questions from the LLM.") # NEW FIELD
...
async def chat_endpoint(request: ChatRequest):
    ...
    try:
        ...
        # Map all fields, including the newly requested ones
        return ChatResponse(
            answer=response_helper.answer,
            sources=response_helper.sources,
            cache_status=response_helper.cache_status,
            suggested_questions=response_helper.suggested_questions # MAPPED NEW FIELD
        )
    except Exception as e:
